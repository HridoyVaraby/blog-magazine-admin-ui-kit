...
export default function CategoryClient({ slug, subcategorySlug, initialData }: CategoryClientProps) {
    ...
    const displayName = subcategorySlug
        ? (data?.subcategory?.name || initialData?.subcategory?.name)
        : (data?.category?.name || initialData?.category?.name);
    const displayDescription = subcategorySlug
        ? `${data?.category?.name || initialData?.category?.name} > ${data?.subcategory?.name || initialData?.subcategory?.name} বিভাগের সকল সংবাদ`
        : `${data?.category?.name || initialData?.category?.name} বিভাগের সকল সংবাদ`;
    ...
    if (error || (!data?.category && !initialData?.category)) {
        return (
            <main className="container mx-auto px-4 py-8">
                <div className="text-center">
        ...
        <main className="container mx-auto px-4 py-8">
            ...
            {posts.length > 0 ? (
                ...
                <div className="text-center py-12">
                    <p className="text-gray-600">
                        {subcategorySlug
                            ? 'এই সাবক্যাটেগরিতে কোনো সংবাদ পাওয়া যায়নি।'
                            : 'এই ক্যাটেগরিতে কোনো সংবাদ পাওয়া যায়নি।'
                        }
                    </p>
                </div>
            )}
